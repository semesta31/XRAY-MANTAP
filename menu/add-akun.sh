...
#buatvless
vlesslinkws="vless://${uuid}@${domain}:443?path=/worryfree&security=tls&encryption=none&type=ws#${user}"
vlesslinknon="vless://${uuid}@${domain}:80?path=/worryfree&encryption=none&type=ws#${user}"
...
trojanlinkws="trojan://${uuid}@${domain}:443?path=/worryfree&security=tls&host=bug.com&type=ws&sni=bug.com#${user}"
#buatshadowsocks
#
cipher="aes-128-gcm"
...
echo -e "=> WS TLS : /worryfree" | tee -a /etc/log-create-user.log
echo -e "=> GRPC   : trojan-grpc" | tee -a /etc/log-create-user.log
echo -e "=> OPOK   : ws://bugcom/worryfree" | tee -a /etc/log-create-user.log
